    ...
    >
      ...
      >
        <Image
          ...
          alt={item.name}
          className={cn(
            "object-cover rounded-full border border-background transition duration-300 h-8 w-8",
          )}
        />
      </motion.div>
    </div>
    ...
    <div className={cn("flex items-center justify-center", className)}>
      ...
      {remainingCount > 0 && (
        <motion.div
          ...
          className={cn(
            "flex items-center justify-center rounded-full border-2 border-background/10 bg-muted text-muted-foreground font-medium",
            size === "sm"
              ? "h-8 w-8"
              : size === "md"
                ? "h-10 w-10"
